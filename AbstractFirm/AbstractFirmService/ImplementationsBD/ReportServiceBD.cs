    ...
    public class ReportServiceBD : IReportService
    {
        ...
        public void SavePackagePrice(ReportBindingModel model)
        {
            ...
            try
            {
                ...
                var products = context.Packages.ToList();
                //создаем таблицу
                ...
                {
                    table.Cell(i + 1, 1).Range.Text = products[i].PackageName;
                        ...
                        if (elem.Blanks.Count() > 0)
                        {
                            ...
                            var excelBorder =
                                excelworksheet.get_Range(excelcells,
                                            excelcells.get_Offset(elem.Blanks.Count() - 1, 1));
        ...
        public void SaveKlientRequests(ReportBindingModel model)
        {
            ...
            table.AddCell(new PdfPCell(new Phrase("Пакет документов", fontForCellBold))
            {
                HorizontalAlignment = Element.ALIGN_CENTER
            });
            ...
            for (int i = 0; i < list.Count; i++)
            {
                ...
                table.AddCell(cell);
                cell = new PdfPCell(new Phrase(list[i].PackageName, fontForCells));
                table.AddCell(cell);
